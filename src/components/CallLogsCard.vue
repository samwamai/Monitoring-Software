                ...
                <div class="col-6 d-flex justify-content-start left-bar-panel">
                    <ul>
                        <li>
                            ...
                            </i>&nbsp;&nbsp;All Call logs {{ groupedList.length }}</a>
                        </li>
                        <li>
                ...
                <div class="col-12 ">
                    <table id="recordsList" class="table table-hover">
                        ...
                        <tbody class="overflow">
                            <tr v-for="calllog in groupedList" v-bind:key="calllog.id">
                                <th scope="row"><span class="fa fa-trash-o"></span></th>
                                <td>
                                    <span v-if="calllog.type == 0" class="fa fa-times missed  missed-icon"
                                        title="Misses Call"></span>
                                    <span v-if="calllog.type == 1" class="fa fa-arrow-left directional-icon incomming-icon"
                                        title="Incomming Call"></span>
                                    <span v-if="calllog.type == 2" class="fa fa-arrow-right directional-icon outgoing-icon"
                                        title="Outgoing Call"></span>
                                </td>
                                <td>{{ calllog.contact }}</td>
                                <td style="color: #005195;">{{ calllog.address }}</td>
...
export default {
    ...
    methods: {
        async fetchCallogs() {
            api.axioWithToken.get(`/calllogs/deviceid/${this.selectedDevice.device_unique_id}`)
                .then((response) => {
